...
function changeHoverColor(element: HTMLElement) {
  const previous = element.style.getPropertyValue("--action-hover");
  const choices = HOVER_COLORS.filter((color) => color !== previous);
  element.style.setProperty("--action-hover", choices[Math.floor(Math.random() * choices.length)]);
}
    ...
    <main className={styles.page} data-opening={openingStage} style={openingStyle}>
      <header className={styles.header} inert={opening}>
        ...
        <div className={styles.actions}>
          <button type="button" aria-label="Reset alphabet" title="Reset alphabet" onMouseEnter={({ currentTarget }) => changeHoverColor(currentTarget)}
            onClick={() => { setComposition(initialComposition()); setSelected(null); setEditorOpen(false); announce("Alphabet reset"); }}><RotateCcw size={14} /></button>
          <a href="https://github.com/derekmeegan/glyphkit" target="_blank" rel="noopener noreferrer" aria-label="Glyphkit on GitHub" title="GitHub"
            onMouseEnter={({ currentTarget }) => changeHoverColor(currentTarget)}>
